...
import { ALL_DISTINCTIONS, PRICE_OPTIONS } from "../utils/filters";
...
function Chip({
  ...
  const base =
    "rounded-md px-2.5 py-1 text-xs font-medium transition border";
  const inactive = "michelin-filter-btn border-michelin-border bg-white text-michelin-gray";
  const activeDefault =
    "michelin-filter-btn-active border-michelin-black bg-michelin-black text-white";
  const activeRed =
    "michelin-filter-btn-red-active border-michelin-red bg-michelin-red text-white";
...
export function FilterSidebar({ filters, onChange, options }: FilterSidebarProps) {
  ...
  return (
    <aside className="flex h-full w-64 shrink-0 flex-col border-r border-michelin-border-light bg-white">
      <div className="border-b border-michelin-border-light p-4">
        <input
          ...
          className="w-full rounded-lg border border-michelin-border bg-white px-3 py-2 text-sm text-michelin-black outline-none placeholder:text-michelin-muted focus:border-michelin-black focus:ring-2 focus:ring-michelin-red/20"
        />
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4">
        ...
        {options.goodFor.length > 0 && (
          <FilterGroup title="Good for" defaultOpen={false}>
